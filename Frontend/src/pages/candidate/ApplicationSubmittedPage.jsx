...
export default function ApplicationSubmittedPage() {
  ...
  const applicationId = searchParams.get("applicationId") || "a1";
  const [data, setData] = useState(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    getApplicationSubmitted(applicationId).then(setData).finally(() => setLoading(false));
  }, [applicationId]);
  ...
  return (
    <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "40px 16px", textAlign: "center" }}>
      ...
      <span style={{ background: "var(--primary-bg)", color: "var(--primary)", padding: "8px 16px", borderRadius: 20, fontSize: 13, fontWeight: 600, marginBottom: 32 }}>
        • {data.statusLabel}
      </span>
